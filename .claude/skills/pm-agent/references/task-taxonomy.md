...
### SECURITY
Any task touching `staff.national_id`, `staff_bank_details` (account number, salary
grade), OAuth/session/auth code, or RBAC middleware in the HR portal.
Keywords: auth, OAuth, session, RBAC, encryption, national id, bank details, salary,
payroll, permissions
→ Must get a `finsec-analyst` review before it can be marked DONE or queued for deploy.
Can be developed in any idle slot; the review gate is what's mandatory, not the slot.
...
5. SECURITY-tagged work (see above) is never DEPLOY-queued or marked DONE without a
   recorded `finsec-analyst` pass — note this explicitly in the task's Notes column
