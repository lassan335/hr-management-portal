...
This is the **HR Management Portal (School)** — a standalone Node/Express + React +
PostgreSQL app (npm workspaces: `frontend/`, `backend/`, `shared/`), replacing a legacy
v7.0 portal (Overtime, Attendance, Leave). See root `README.md` and
`C:\Users\LASSAN\.claude\plans\luminous-juggling-lantern.md` for the full build plan.

- **Who executes deploys?** The `release-engineer` agent (`.claude/agents/release-engineer.md`),
  runbook at `docs/DEPLOY.md`. Hosting is **not yet chosen** — treat any `deploy`/`queue`
  command as blocked on that decision until `docs/DEPLOY.md`'s Deployment coordinates
  section is filled in.
- **Real build/migrate commands:** `npm run build --workspace=backend`,
  `npm run build --workspace=frontend`, `npx prisma migrate deploy` (run from `backend/`).
- **Security review before deploy:** any task touching `staff.national_id`,
  `staff_bank_details` (account number, salary grade), auth/OAuth code, or RBAC
  middleware is a **SECURITY** task type (see `references/task-taxonomy.md`) — route it
  through `finsec-analyst` before it can be marked DONE or queued for deploy.
- **Branch convention:** feature branches off `main`; PRs target `main`.
