...
This is the **HR Management Portal (School)**: React+TS+Tailwind frontend, Node.js +
Express + TypeScript backend, PostgreSQL via Prisma, npm workspaces
(`frontend/`, `backend/`, `shared/`). There is no payments/wallet flow — the
crown-jewel data here is employee PII and payroll data instead of money movement.
Re-verify these against the actual code each review (they may drift as modules are built):

- **"Money" representation → payroll fields.** There are no transactions/wallets.
  The equivalent sensitive numeric data is `staff_bank_details.salary_grade` and
  bank account numbers. Flag any of these ever logged, returned in a non-HR/Admin
  API response, or stored/compared as plaintext instead of via
  `backend/src/lib/encryption.ts`.
- **Authentication mechanism.** Google OAuth 2.0 via `passport-google-oauth20`. On
  callback, the domain restriction check (Google `hd` claim / verified email domain
  vs `ALLOWED_GOOGLE_DOMAIN`) MUST happen server-side before a session is issued —
  verify it cannot be bypassed by a crafted callback or a personal Gmail account.
  Session is a signed JWT in an httpOnly+Secure+SameSite cookie — verify the JWT
  secret comes from env, the algorithm is not `none`/weak, and expiry is enforced.
  **`DEV_BYPASS_AUTH`** is a local-only escape hatch — treat any code path where it
  can be true when `NODE_ENV !== 'development'` as a P0 authentication-bypass finding.
- **Authorisation gates.** `requireRole(...)` + `scopeToDepartment` middleware in
  `backend/src/lib/rbac.ts`. Staff → own record only; HOD → own department (limited
  staff-detail fields, full overtime/attendance/leave); HR/Admin → everything
  including `staff_bank_details` and `national_id`. Verify EVERY route/controller
  touching these enforces the gate server-side — never trust a frontend role check.
- **Mutation pattern.** Express controllers under `backend/src/modules/*`. Confirm
  writes to `staff`, `staff_bank_details`, `overtime_requests`, `leave_requests`,
  and any approval-chain transition write an `audit_log` entry (actor, action,
  entity, before/after) — missing audit logging on an approval or a payroll/PII
  edit is an audit-trail finding (Domain 7).
- **PII / identity flow (this project's KYC-equivalent).** Employee identity data
  (`national_id`, DOB, documents) is captured at onboarding/self-service edit and
  requires HR approval for sensitive-field changes (`staff_edit_requests`). Verify
  the approval gate is enforced server-side, not just hidden in the UI.
- **Encrypted-at-rest fields.** `staff.national_id`, `staff_bank_details.account_number`,
  `staff_bank_details.salary_grade` — via `backend/src/lib/encryption.ts`
  (AES-256-GCM, key from `ENCRYPTION_KEY` env var). Verify: the key is never
  hardcoded/committed, ciphertext (not plaintext) is what's persisted, decryption
  only happens inside HR/Admin-authorized query paths, and these fields are never
  included in a Staff/HOD-facing serializer even in encrypted form.
- **File uploads/downloads.** `multer` disk storage under `backend/uploads/`. Files
  must NEVER be statically served — only via the authenticated `GET /api/files/:fileId`
  route that re-checks ownership/role. Flag any static file-serving middleware
  pointed at `uploads/`, and any path-traversal risk in the file-id lookup.
- **ZKTime import job.** `backend/src/jobs/zktimeImport.ts`, fed by a file watcher
  on `backend/import-watch/incoming/` and a manual upload endpoint. Verify the
  upload endpoint is HR/Admin-gated, uploaded files are validated (type/size) before
  parsing, and parser errors on malformed rows don't crash the watcher or silently
  drop records (unmatched rows must land in the review queue, per spec).
- **Client/server boundary.** Confirm `ENCRYPTION_KEY`, the OAuth client secret, and
  the JWT signing secret never reach `frontend/` code or bundles.
- **Secrets & DB config.** `ENCRYPTION_KEY`, `ALLOWED_GOOGLE_DOMAIN`,
  `GOOGLE_CLIENT_SECRET`, JWT secret, and the Postgres connection string must all
  come from env — never hardcoded or committed. `docs/DEPLOY.md`'s credential
  inventory table is the source of truth for where each one lives.
- **Stack specifics.** Prisma (parameterised by default — flag any `$queryRawUnsafe`
  with interpolated user input), Zod (or equivalent) for server-side request
  validation on every controller, not just frontend form validation.

Payments/wallet-specific sub-checks (double-spend, transaction IDOR, PAN
tokenisation) are **N/A** to this project — say so explicitly rather than inventing
a finding, but re-flag immediately if a payments feature is ever introduced.
